...
% ─── Abstract ──────────────────────────────────────────────
\begin{abstract}
We present UNITARES, a governance framework for heterogeneous fleets
of autonomous AI agents. Each agent carries a four-dimensional state
vector $\eisv = (E, I, S, V)^\top$ intended to support continuous
governance rather than discrete after-the-fact filtering. The central
design constraint is heterogeneity: the populations we govern include
embodied agents with sensor-driven state, persistent autonomous
services, session-bounded coding assistants, and ephemeral parser
agents. These do not share an output modality, a tempo, or a healthy
operating point, so fleet-wide normalization is the wrong target.
...
$S$ as response-distribution entropy, $I$ as context-response mutual
information, $E$ as negative variational free energy or a resource-rate
proxy, and $V$ as an accumulated free-energy residual. Second, it makes
normalization class-conditional: scale constants and healthy operating
points are functions of an agent class keyed on existing identity tags,
with fleet-wide defaults retained only as fallback behavior.

Because UNITARES was already deployed, re-grounding the coordinates
raised a live-systems problem in addition to a mathematical one. We
therefore introduce a pipeline-ordering migration mechanism that
preserves governance behavior while allowing grounded values to populate
canonical response fields. Stability of the dynamics is preserved under
the reformulation and established by contraction analysis
(Appendix~B).
...
\texttt{low} basin---direct empirical support for the
homogenization-failure-mode argument. Per-class calibration of the
remaining scale constants and higher-tier estimators (logprob and
multi-sample) are deferred to subsequent work.
\end{abstract}
...
The framework's earliest formulations
\cite{wang2026unitares-v3,wang2026unitares-v5} were thermodynamically inspired but
phenomenologically constructed: the coordinates were named after
physical quantities that they did not actually compute. This paper
re-grounds the coordinates information-theoretically while
preserving the dynamics, the verdicts, and the production
deployment. We further address a design constraint that earlier
formulations did not surface: the populations the framework
governs are heterogeneous. They differ in tempo by four orders of
magnitude, in output modality (text, sensor, system action), and
in healthy operating point. A governance framework that collapses
this population to a single state distribution---what we call
\emph{cosmological soup}---loses signal exactly where signal
matters. The contributions of this paper are organized around
these two reframings.
...
\item \textbf{Information-theoretic reinterpretation of EISV}
  (\Cref{sec:grounding}): We replace the earlier thermodynamic labeling
  with computable information-theoretic semantics for the EISV
  coordinates, together with tiered estimation recipes and provenance
  tags.
...
\item \textbf{Production deployment snapshot and verdict counterfactual}
  ...
  (predominantly into the \texttt{low} basin) being empirical
  support for the homogenization-failure-mode argument of contribution
  $1$.
\end{enumerate}
...
Shannon entropy and mutual information are standard tools for
...
agents minimize expected surprise; the active-inference
literature \cite{dacosta2020active,sajid2021active,smith2022step,parr2022active}
develops the computational machinery for the free-energy quantity
used in our $E$ coordinate. UNITARES adapts these
information-theoretic quantities to the governance setting and
treats the choice of computation tier (logprob, multi-sample,
heuristic) as a deployment configuration rather than an
assumption. We use the variational form rather than the
discrete-state-space active-inference form because the agents
under governance do not generally expose discrete latent
state-spaces to the governance layer.
...
headline contribution; see Appendix~B.
...
UNITARES share the goal of detecting value-level drift, they differ
...
Claude / Codex sessions & --- & text response & $40$--$80$ Hz & session-bounded \\
...
The contraction-theoretic stability result presented in v5 of this
work \cite{wang2026unitares-v5} demonstrates that the
EISV dynamics are well-behaved under standard control-theoretic
conditions. That result is preserved here (\Cref{app:contraction-proof})
but is no longer the headline contribution. Stability of the
dynamics is a property of any reasonable phenomenological
governance equation; what distinguishes UNITARES is that the same
dynamics describe a heterogeneous population without collapsing the
classes that define them. The v6 framing makes that distinction
explicit: the contribution is the class-conditional grounding, not
...
Earlier formulations of UNITARES used wider nominal bounds
($S_{\max} = V_{\max} = 2$) without empirical justification.
Production data
(\Cref{sec:experiments}) shows $V \in [-0.089, 0.007]$ and
$S \approx 0.08$ across the active fleet, two orders of magnitude
inside the wider bounds. The grounded definitions of
\Cref{sec:grounding} normalize each coordinate to $[0,1]$
(or $[-1,1]$ for $V$) by construction via the scale constants of
\Cref{sec:class-calibration}, so the wider bounds were never
load-bearing. v6 narrows the state space to $[0,1]^3 \times [-1,1]$
to match the normalization.
\end{remark}
...
(\Cref{sec:coherence}), $\norm{\deta}$ is the behavioral--epistemic drift norm
...
free-energy debt of \Cref{eq:V-grounded}. The arithmetic is
unchanged from earlier UNITARES formulations; the meaning is
upgraded.
\end{remark}
...
at $V = 0$. The dual presentation reflects the dual-compute
migration of \Cref{sec:migration}: the grounded form is reported
in canonical payload slots; the operational form remains the
quantity the dynamics consume until the Phase~3 swap migrates the
ODE coefficients onto the grounded coherence.
...
The \eisv\ system presented in \Cref{sec:dynamics} is
phenomenological: each coordinate has a physical interpretation and the
coupling structure is thermodynamically motivated, but the coordinates
are not themselves thermodynamic quantities. This is an honest
framing---we state it plainly in
\Cref{sec:thermodynamic-connections}---but it leaves every constant
hand-tuned and every threshold empirically discovered rather than
derived. This section upgrades the phenomenology to a computable
information-theoretic grounding without changing the dynamics of
\Cref{sec:equations}.
...
\citep{friston2010free} gives a coherent top-level dynamic: agent
drift corresponds to rising variational free energy, and governance
becomes free-energy monitoring. The FEP inherits a derivation from
statistical mechanics through variational Bayes, giving us a real
ancestry for the free-energy quantity we already had in $V$.

Alternatives considered and rejected: pure control-theoretic
Lyapunov framing (abandons the thermodynamic lineage entirely);
...
Tier~3 retains the legacy logistic heuristic.
...
outcomes (low surprise) and low otherwise. A deployment-configurable
alternative is the \emph{resource-rate} form
$E_{\text{resource}} = (\text{tokens}/s) / (\text{tokens}/s)_{\max}$,
which approximates $-F$ under stationary conditions and is directly
measurable from inference metadata. Both forms expose themselves via
an \texttt{e\_source} annotation.
...
is a measured quantity, not a guess. For each constant
$\kappa \in \{S_{\text{scale}}, I_{\text{scale}}, E_{\text{scale}}, \norm{\Delta}_{\max}\}$,
the implementation records: the numerical value, the ISO date on
which it was set, the corpus size over which it was measured, the
percentile basis (typically 90th or 95th of a healthy-fleet
distribution), and a provenance tag in
$\{\text{placeholder},\text{measured},\text{derived}\}$. Phase~1
ships placeholder values tagged explicitly; Phase~2 replaces them
with measured values per the protocol in the implementation plan
with results reported in subsequent work.

\subsection{Heterogeneity as a First-Class Constraint}
\label{sec:grounding-heterogeneity}

The framework must not collapse to fleet-wide constants. The
deployments that motivate UNITARES are explicitly heterogeneous: an
embodied creature reading I$^2$C sensors at $0.5$ Hz, a cron-driven
janitorial agent that wakes once per 30 minutes, an analytical
observer that streams over a websocket, a session-bounded coding
assistant generating tokens at $40$--$80$ Hz, and ephemeral
parser agents that exist for milliseconds. These do not share a
healthy operating point. They do not share a tempo. Their
``outputs'' are not in the same units---an embodied agent's $E$
should not be normalized against a tokenizer's throughput envelope.

Phase~1 ships fleet-wide constants explicitly tagged
\texttt{provenance="placeholder"}; this is a scaffolding choice, not
a claim. The framework's normalization layer must in production be
\emph{class-conditional}: scale constants and the healthy baseline
point $(E,I,S)_{\text{healthy}}$ are functions of an agent class,
not global parameters. UNITARES already carries the keying material
for class assignment via existing identity tags
($\texttt{embodied}$, $\texttt{autonomous}$, $\texttt{persistent}$,
$\texttt{ephemeral}$, etc.) and labels (e.g., \texttt{Lumen},
\texttt{Vigil}, \texttt{Sentinel}); Phase~2 calibration partitions
the reference corpus along these axes and computes class-conditional
constants. The default for an unclassified agent is the
fleet-wide constant, retained as a safe fallback.

\begin{remark}[Why this matters]
A coherence baseline computed across a homogenized fleet is the
information-theoretic equivalent of a cosmological soup: maximally
entropic, no class structure, no distinguishability. A divergent
agent (high $S$ by design) flagged as incoherent against an
analytical baseline is not a governance signal---it is a category
error introduced by the choice of $\norm{\Delta}_{\max}$. The
subsequent work will report
class-conditional values and the population structure they
induce.
\end{remark}

\subsection{Dual-Compute Migration}

Phase~1 is \emph{dual-compute}: every check-in reports both the
grounded value (under canonical symbols $E, I, S, C$) and the
legacy value (under $E_{\text{legacy}}$, etc.), with source
annotations on each grounded coordinate
(\texttt{e\_source}, \texttt{i\_source}, \texttt{s\_source},
\texttt{coherence\_source}) identifying which tier produced the
value. Governance decisions---verdicts,
basin classification, risk thresholds---continue to read the legacy
values, which is enforced by the pipeline: gating runs before the
grounding enrichment in the server's request pipeline. Phase~2
measures divergence between grounded and legacy on the reference
corpus. Phase~3 migrates gating to the grounded values once
...
The EISV dynamics of \Cref{sec:dynamics}, under linear information
mode (\Cref{sec:saturation}), are globally exponentially stable.
Specifically, the system is contracting with rate
$\alpha_c > 0$ under a Riemannian metric
$M = \operatorname{diag}(0.1, 0.2, 1.0, 0.08)$, with
$\alpha_c = 0.019$ certified by Gershgorin analysis at production
parameters. This rate is a conservative lower bound; observed
convergence in the production deployment is faster
...
class-indexed, and the contraction analysis carries through
class-by-class with each class's own $\alpha_c(c)$. The
equilibrium $\eisv^*$ becomes the class-conditional healthy
operating point of \Cref{def:scale-map} in deployments where
calibration has been performed. Cross-class network
...
\begin{remark}[Phase 3 re-certification]
\label{rem:phase3-contraction}
Phases 1 and 2 (\Cref{sec:migration}) preserve the operational
$\cv = \tanh$-based form that the contraction proof of
\Cref{app:contraction-proof} consumes through the off-diagonal
bound $\beta_I C'_{\max} = 0.15$. In Phase 3, the dynamics
themselves are rewired to consume a grounded coherence
(\Cref{eq:C-manifold} or \Cref{eq:C-KL}), which changes the
relevant Jacobian entries: $\partial \cv / \partial V$ is replaced
by derivatives with respect to $(E, I, S)$, altering the structure
of the off-diagonal bound on which the Gershgorin argument
depends. The contraction certificate therefore does not carry over
to Phase 3 unconditionally; re-certification at the new operating
point with the new coherence derivative is required. The proof
structure of \Cref{app:contraction-proof} adapts, but the
numerical rate $\alpha_c$ will change.
\end{remark}
...
The information dynamics of \Cref{sec:dynamics} use a logistic self-regulation term:
\begin{equation}
...
An earlier design used three tiers: approve/revise/reject. We
abandoned the three-tier form on design grounds rather than from a
measured oscillation study: a \texttt{revise} verdict carries no
information about \emph{what} to revise, which makes it operationally
indistinguishable from \texttt{proceed} for agents whose next action is
unchanged by the verdict's content. The two-tier design replaces the
ambiguous middle state with a binary contract (proceed / pause) and
delegates the nuance that \texttt{revise} attempted to encode to the
PID governor's continuous threshold adaptation, which does carry a
derivative signal. A controlled oscillation-rate comparison between
the two- and three-tier forms remains future work.
\end{remark}
...
available), bypassing the PID loop entirely. Neighbor-pressure
aggregation (\Cref{eq:neighbor}) still applies across the
ephemeral-class cohort, which recovers some of the class-wide
adaptation the per-agent loop would have provided for longer-lived
agents. The cost is that ephemeral classes do not benefit from
per-agent threshold learning; the benefit is that a 5~ms parser is
not blocked waiting for a 20-step window to populate. A principled
treatment of sub-window-lifetime agents---possibly by pooling the
PID state across a class cohort rather than per-agent---is future
work.
...
Earlier UNITARES formulations described the quantity below as
``ethical drift.'' The label overstates what is actually
instrumented. The four signals defined in \Cref{def:drift} are
behavioral (decision consistency, coherence departure) and
epistemic (calibration gap, complexity self-estimation error);
none of them measure the agent's values in a normative sense.
A faithful rename is therefore \emph{behavioral--epistemic drift}.
We retain the symbol $\deta$ and the macro name for continuity
with the deployed code and prior publications. Two adjacent axes
that the present vector does \emph{not} instrument, but which a
complete treatment would include as distinct channels, are:

\begin{description}
\item[Emotional / affective drift.] For embodied agents with
  sensor-driven state (Lumen in the production fleet), proxies
  such as prosody, expression, or heart-rate-variability deltas
  would populate a $\deta^{\text{aff}}$ channel. This is
  instrumentable but not currently wired into the governance
  server.
\item[Ethical / value drift.] Deviation from a declared value
...
Both are natural extensions to the vector defined below; both are
out of scope for the present paper. The remainder of this section
grounds the behavioral--epistemic vector in four measurable signals,
each bounded in $[0,1]$.

\begin{remark}[Pre-grounding form]
The four signals defined below carry over from earlier formulations
unchanged. Under the information-theoretic frame of
\Cref{sec:grounding}, two of them have natural information-theoretic
re-derivations: $\eta_{\text{cal}}$ (calibration deviation) is a
miscalibration KL divergence between predicted and empirical outcome
distributions, and $\eta_{\text{coh}}$ (coherence deviation) is a
KL divergence from the class-conditional baseline distribution of
\Cref{sec:grounding}. We retain the distance-based forms here for
continuity with the deployed system; the KL re-derivations
constitute additional grounding work for subsequent versions.
\end{remark}
...
\begin{remark}[Cold-start homogenization: a bootstrapping trade-off, not a steady-state property]
\label{rem:cold-start}
The fallback-to-$\Sigma(c_{\text{default}})$ behavior during
the first $N_{\min}$ turns of a new class's existence is, formally,
the homogenization failure mode of \Cref{sec:heterogeneity}
applied to that single class: the class is being governed against
a fleet-wide envelope while its own is still being measured. We
do not claim this is costless. We do claim it is a bounded-duration
bootstrapping choice rather than a steady-state commitment:
(i)~the provenance tag $\rho = \text{placeholder}$ makes the
regime explicit in every check-in payload; (ii)~once the class
has accumulated $N_{\min}$ healthy turns, the measurement pipeline
transitions the provenance to \texttt{measured} and the class
exits the homogenization regime; (iii)~operators deploying a new
class with known-different tempo or modality are encouraged to
hand-author an initial $\Sigma(c)$ with provenance
\texttt{derived} rather than inherit defaults, and the framework
exposes this as a supported entry path. The tension the critic
identifies is real: a class in its cold-start window is \emph{not}
enjoying the benefits of class-conditional governance. The
framework's response is to make that state observable rather than
to paper over it.
\end{remark}
...
Re-grounding the EISV state vector raised an engineering problem
distinct from the mathematical one. The framework was already in
production: governance verdicts were being issued, dashboards were
displaying live metrics, calibration data was accumulating, and
external clients had pinned their integrations against the
existing payload schema. We needed to introduce computable grounded
quantities without invalidating any of this. This section documents
the mechanism that enabled the migration; we believe it
generalizes to any deployed agent framework whose semantics need
to shift while service continues.
...
\item[Phase 2: Calibration.]
  ...
  legacy values is characterized; the empirical appendix reports
  the result.
...
phases: a \emph{phases} phase that computes governance state and
issues the verdict, and an \emph{enrichments} phase that augments
the response payload with secondary signals. The enrichments
phase is registered as an ordered list; entries with lower order
run first. We register the grounding computation as an enrichment
at order $75$, downstream of every entry that contributes to the
verdict and basin classification but upstream of presentation
enrichments such as the human-readable summary and the broadcaster
event.

The result: by the time the grounding enrichment runs, the verdict
has already been computed---using the legacy values produced by
the phases phase. The grounding enrichment then swaps grounded
...
\begin{remark}[Idempotency]
The enrichment checks for the presence of $E_{\text{legacy}}$
before swapping; on a second invocation against the same context
it returns immediately. This guards against double-application in
testing and during pipeline replay.
\end{remark}

\subsection{What Doesn't Get Grounded in Phase 1}

The void coordinate $V$ is not dual-computed. As discussed in
\Cref{sec:grounding}, $V$ is structurally an accumulator; its
grounded reinterpretation as accumulated free-energy debt
\Cref{eq:V-grounded} is a relabeling of an existing quantity, not
a new computation. No source annotation, no $V_{\text{legacy}}$
twin. The internal naming
($\texttt{void} \to \texttt{free\_energy\_debt}$ in subsequent
revisions) is a cosmetic change tracked separately from the
dual-compute migration.

Tier-1 (logprob-based) and tier-2 (multi-sample) computations are
present in the codebase as interface stubs that raise
\texttt{NotImplementedError}; the dispatcher catches and falls
through to tier-3 (heuristic / resource / manifold). This keeps
the dispatcher surface stable while the inference-layer
instrumentation needed to populate logprobs lands separately,
typically as plugin work outside the governance server.
...
continues. The required structure is:
\begin{enumerate}
  \item A request pipeline with explicit ordering between
    decision-producing stages and presentation-producing stages.
  \item A canonical payload slot for the coordinate being
    re-grounded.
  \item Identity (or session) keying that allows audit
    consumers to tag rows by the migration phase active when the
    row was produced.
\end{enumerate}
Frameworks that conflate decision and presentation into a single
non-ordered handler cannot apply this mechanism directly; they
must first separate the two concerns. We argue this separation is
worth doing independently of any migration: it makes governance
decisions auditable and presentation choices revisable in
isolation.
...
pipelines see $\tilde{r}$; refusal logic continues to read $r$. The
audit log tags each row with the active migration phase, so a
divergence study over a held-out corpus can characterize where
$\tilde{r}$ and $r$ would disagree before any Phase 3 cutover. The
mechanism is identical; only the coordinate being re-grounded
changes.
...
\subsection{Beyond Mean-Square Contraction: What a Fokker--Planck Treatment Would Add}
\label{sec:fp-gap}

The mean-square bound of \Cref{thm:stochastic} is a worst-case
contraction statement. It does not produce a verdict-level
false-alarm rate. The sharper operational question---\emph{given
an agent in the healthy regime, what is the probability that
stochastic excursions cross the governance threshold
$\Phi < 0$ and trigger a spurious pause verdict?}---requires the
stationary distribution of the SDE.

The Fokker--Planck equation for $p(\eisv, t)$ under
\Cref{eq:sde} is
\[
  \partial_t p = -\nabla \cdot (f\,p)
  + \tfrac{1}{2}\sum_{i,j} \partial_i \partial_j(D_{ij}\,p),
  \qquad D = G G^\top.
\]
With the production diffusion $G = \operatorname{diag}(0,0,\sigma,0)$,
the noise is injected only on the $S$ channel, so $D$ has a single
non-zero entry $D_{SS} = \sigma^2$. The stationary distribution
$p_{\ast}(\eisv) = \lim_{t \to \infty} p(\eisv, t)$---once
derived---would enable direct computation of
\[
  P_{\text{FA}}
  = \int_{\{\eisv : \Phi(\eisv) < 0\}} p_{\ast}(\eisv)\, d\eisv,
\]
the false-alarm rate at the \texttt{pause} boundary for an agent
whose deterministic trajectory sits in the healthy basin. Related
FP frameworks for LLM drift~\cite{carson2025stochastic} give
closed-form first-passage times for one-dimensional severity
processes.

We do not carry out the 4-D Fokker--Planck derivation here. The
deterministic contraction analysis shows variance is bounded; the
empirical verdict-flip counterfactual in \Cref{sec:experiments}
shows the class-conditional form changes verdicts at the population
level; a proper $P_{\text{FA}}$ bound requires either an analytical
FP treatment under a suitable reduction (e.g., marginalizing to
the $S$ subsystem that actually carries the noise, where the FP
equation is one-dimensional and tractable) or a numerical
integration of the SDE trajectory ensemble against the governance
threshold. Either is substantial enough work to be its own
paper; we flag it here as the sharpest remaining rigor gap in the
present treatment.
...
\subsection{Saturation and Regime Diagnostics}
...
\subsubsection*{Regime Distribution}
...
\textbf{A note on temporal seam.} The fleet-aggregate equilibrium
statistics in \Cref{tab:eisv-observed} were measured on a snapshot
of February~20,~2026; the per-class measurement in this
subsection and the verdict counterfactual in
\Cref{sec:verdict-counterfactual} were measured on a 30-day
window ending April~18,~2026. Both draw from the same
continuously-running deployment, but the populations have grown
(903~$\to$~3{,}286 registered identities) over the interval. We
report both measurements honestly because the older snapshot
underlies the reported fleet-aggregate statistics and the newer
one underlies the per-class measurement; reconciling them to a
single date would require either re-deriving the fleet-aggregate
statistics on April~18 (which would invalidate the prior
discussion in \Cref{sec:experiments}) or running the per-class
measurement on the smaller February snapshot (where some classes
would fall below $N_{\min}$). The temporal seam is real and
acknowledged.
...
empirically supports the homogenization argument of
\Cref{sec:heterogeneity}: a fleet-wide constant chosen to cover
the worst case (or chosen arbitrarily) has no discriminating
...
legacy fleet-wide form was systematically classifying state
vectors as healthier than their class-conditional position
warrants.}
...
class-conditional grounded form. This is the
homogenization-failure-mode argument of
\Cref{sec:heterogeneity} made empirical at the gating layer: a
fleet-wide constant chosen to span the worst-case envelope is
necessarily permissive relative to the tighter envelopes of
specific classes.
...
but makes the energy channel less responsive to behavioral--epistemic drift.
...
$w_I$ & $-0.12$ & promotes proceed (small, since $I$ is high) \\
$w_S$ & $-0.08$ & promotes proceed (small, since $S$ is low) \\
$w_V$ & $-0.03$ & negligible \\
...
\subsection{Why We Left Thermodynamics}
\label{sec:thermodynamic-connections}

Earlier UNITARES formulations
\cite{wang2026unitares-v3,wang2026unitares-v5} described the EISV state vector as
``thermodynamic'' on the strength of an analogy: $E$ resembled
internal energy, $S$ resembled entropy, $V$ resembled Helmholtz
free energy. This paper deliberately abandons that framing. The
analogy was a labeling convenience, not a derivation; no
thermodynamic law (conservation, second law, fluctuation-dissipation)
held in the EISV equations, and continuing to invoke
thermodynamic vocabulary risked claiming structure that the math
did not support.

The grounded coordinates of \Cref{sec:grounding} are
information-theoretic. Their formal ancestry is Shannon
information theory \cite{shannon1948} and the variational
free-energy principle \cite{friston2010free}---both of which have
genuine derivations from probability theory and statistical
mechanics respectively. The two ancestries connect: Friston's
free energy is itself a Helmholtz-style quantity arising from
variational Bayes, so $V$ as accumulated free-energy debt
(\Cref{eq:V-grounded}) shares mathematical heritage with
Helmholtz free energy without requiring us to claim a direct
thermodynamic mapping. The honest correspondence is now:
...
We retain the symbols
$\{E, I, S, V, C\}$ for continuity with prior UNITARES work and
deployment compatibility, but their semantic load is now the
right column.
...
appear in \Cref{sec:experiments}; class-conditional empirical
results are deferred to subsequent work following the
measurement protocol of \Cref{sec:class-calibration}.

The framework's adaptive governor (\Cref{sec:governor}), ethical
behavioral--epistemic drift vector (\Cref{sec:ethical-drift}), and multi-agent
extensions (\Cref{sec:network}) carry over from earlier
formulations. The honest accounting of what changed and what was
preserved appears in
\Cref{sec:thermodynamic-connections}: we kept the
$\{E, I, S, V, C\}$ symbols and the dynamics; we replaced what
they mean and how they are normalized.
...
\bibitem[Wang(2026a)]{wang2026unitares-v3}
K.~Wang.
\newblock {UNITARES}: Thermodynamic governance for autonomous {AI} agents (v3).
\newblock Working draft, CIRWEL Systems, 2026.

\bibitem[Wang(2026b)]{wang2026unitares-v5}
K.~Wang.
\newblock {UNITARES}: Contraction-theoretic governance of autonomous agent thermodynamics (v5).
\newblock Working draft, CIRWEL Systems, 2026.
...
\bibitem[Wang et~al.(2025)]{wang2025mi9}
...
\centering
\footnotesize
\caption{Grounding scale constants (\Cref{sec:scale-constants}). All values shipped as placeholders pending class-conditional measurement per the protocol of \Cref{sec:class-calibration}; subsequent work replaces them with measured values.}
...
Manifold rad. & $\norm{\Delta}_{\max}$ & 1.8 & --- & placeholder \\
...
per \Cref{def:scale-map}. All entries are currently
\texttt{placeholder}, an explicit signal that they are
scaffolding values awaiting the class-conditional measurement
protocol; subsequent work replaces this table with measured
values that include the date and corpus size for each entry.
...
equations \eqref{eq:dE}--\eqref{eq:dV} (using linear $g_I$ mode,
see \Cref{sec:saturation}):
\begin{equation}
J = \begin{pmatrix}
