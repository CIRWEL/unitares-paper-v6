...
UNITARES's ethical drift vector (\Cref{sec:ethical-drift}) couples
...
($-\mu S$) and is driven up by ethical drift and task complexity;
...
$H(q)$.
Tier~3: retain the legacy heuristic (complexity, ethical drift) as a
...
from unpredictable task inputs. An alternative Fokker--Planck framing
of LLM-severity drift as a critical process appears
in~\cite{carson2025stochastic}; we retain the SDE framing here because
it composes directly with the contraction analysis of
\Cref{sec:contraction} via Pham et al.~\cite{pham2009stochastic}. We
model this as an It\^{o} SDE:
\begin{equation}
...
drift vector (\Cref{sec:ethical-drift}), and multi-agent
