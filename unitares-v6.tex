...
These results should be read as partial deployment evidence for the
framework and migration strategy, not as full empirical validation of
the class-conditional grounding. Per-class calibration results and
higher-tier estimators (logprob and multi-sample) are deferred to
subsequent work.
\end{abstract}
...
\item \textbf{Partial deployment evidence}
  (\Cref{sec:experiments}): We report a production snapshot of the
  deployed system to characterize operating regimes, saturation
  behavior, and observed fleet dynamics. These results are descriptive
  and migration-oriented; full class-conditional empirical validation is
...
calibration protocol is specified here, but the per-class empirical
constants are not yet reported. Second, the highest-fidelity estimators
for grounded quantities (logprob-based and multi-sample computation)
remain future instrumentation work; the current deployment relies on
degraded-mode fallbacks where those signals are unavailable. Third, the
deployment results in \Cref{sec:experiments} are presented as a single
descriptive production snapshot rather than a controlled validation
study across stable parameter regimes.
...
agents minimize expected surprise. UNITARES adapts these
...
The tags
$\{\texttt{embodied}, \texttt{autonomous}, \texttt{persistent}, \texttt{ephemeral}, \texttt{pioneer}\}$
...
legacy value (under $E_{\text{legacy}}$, etc.), accompanied by
source annotations $\{\texttt{e\_source}, \texttt{i\_source},
\texttt{s\_source}, \texttt{coherence\_source}\}$ identifying which
tier produced the grounded value. Governance decisions---verdicts,
...
\Cref{sec:grounding} preserves stability. The dynamics
$\dot{\eisv} = f(\eisv, t)$ are unchanged; only the meaning of
the coordinates changes. The contraction certificate of
\Cref{thm:contraction} therefore carries over without
modification, and the equilibrium $\eisv^*$ becomes the
...
\item[Phase 1: Dual-compute.]
  ...
  C_{\text{legacy}}\}$. Source annotations
  $\{\texttt{e\_source}, \texttt{i\_source}, \texttt{s\_source},
  \texttt{coherence\_source}\}$ identify which computation tier
  produced each grounded value (logprob / multisample / heuristic
  / resource / manifold / KL / FEP).
...
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Information theory / FEP} & \textbf{UNITARES coordinate} \\
\midrule
...
KL or manifold distance from healthy & Coherence $C$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{@{}llrll@{}}
\toprule
\textbf{Constant} & \textbf{Symbol} & \textbf{Val.} & \textbf{Pctl} & \textbf{Provenance} \\
\midrule
Entropy scale & $S_{\text{scale}}$ & 3.0 & --- & placeholder \\
Mutual info scale & $I_{\text{scale}}$ & 2.0 & --- & placeholder \\
Free energy scale & $E_{\text{scale}}$ & 1.0 & --- & placeholder \\
Manifold radius & $\norm{\Delta}_{\max}$ & 1.8 & --- & placeholder \\
Tokens/sec env. & $(\text{tok/s})_{\max}$ & 200 & --- & placeholder \\
\bottomrule
\end{tabular}
\end{table}
